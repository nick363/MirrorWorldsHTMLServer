...
socket.once('firstupdate', function(fullListOfUsers)
{
	...
	{
		var userBundle = document.createElement('Group');
		userBundle.setAttribute("id", key + "Bundle");
		
		scene.appendChild(userBundle);
		
		var userCam = document.createElement('Viewpoint');
		userCam.setAttribute("position", -5 + " " + -5 + " " + 0);
		userCam.setAttribute("orientation", 0 + " " + 0 + " " + 0 + " " + 0);
...
socket.on('update', function(updatedUser)
{
	...
	//Get Avatar Camera Data
	var userCam = document.getElementById(updatedUser[0] + "Camera");
