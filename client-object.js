...
window.addEventListener('keypress', function(e) {
	...
	if(e.keyCode === 49) {
		...
		if (thirdCam.getAttribute("set_bind")) {
			...
			thirdCam.removeChild(bundle);
			
			//avatar.setAttribute('translation', 0 + " " + 0 + " " + 5);
			
			var cPos = bundle.getAttribute("translation");
			var cRot = bundle.getAttribute("rotation");
			firstCam.setAttribute("position", cPos);
			firstCam.setAttribute("orientation", cRot);
			
			firstCam.appendChild(bundle);
	...
	if(e.keyCode === 51) {
		...
		if (firstCam.getAttribute("set_bind")) {
			...
			firstCam.removeChild(bundle);
			
			//avatar.setAttribute('translation', 0 + " " + 0 + " " + 0);
			
			var cPos = bundle.getAttribute("translation");
			var cRot = bundle.getAttribute("rotation");
			thirdCam.setAttribute("position", cPos);
			thirdCam.setAttribute("orientation", cRot);
			
			thirdCam.appendChild(bundle);
...
socket.once('firstupdate', function(fullListOfUsers)
{
	...
	for (var key in fullListOfUsers)
	{
		...
		if(current[0] == name) {
			...
			var camera = x3d.runtime.getActiveBindable("Viewpoint");
			console.log("Active Camera :", camera.getAttribute("id"));
			
			camera.appendChild(userBundle);
